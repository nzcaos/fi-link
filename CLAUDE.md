...
**Member-attribute grid (`LIST_ATTRIBUT.display_row`).** The child's `member` attributes are not stacked with inline labels — they render as a properly aligned grid. The child name is its **own table column** (`col-name`), vertically centred, auto-sized to the longest name in the list (`white-space:nowrap`). The member attributes occupy a **second table column** (`col-member`) holding a fixed-column CSS grid, so the values line up under their headers and across every child. Because not all child attributes fit on one line, each attribute carries a `display_row` (1, 2, …, default 1) on the LISTTEMPLATE that places it on a grid row; within a row, `position` orders the columns. The grid column count is the widest display-row; shorter rows are padded so columns stay aligned. The header (the same rows × columns, in the `<thead>`) is built by `visibility.member_grid_header(template)`; the per-child value grids by `build_composed_rows` (`member_grid`: one list of `{attr, value}` cells per display-row, `value=None` where hidden → "—", `None` cells for padding) — both off the shared `_group_member_display_rows`, so they never drift. `lists/_child_cell.html` renders the two child cells. Associate attributes keep their single top-level header row (one column per attribute) unchanged.
...
**Authoring is Django-Admin / super-admin only** (no self-service builder) — same as before. The end-user surface is the signup flow. A **clone** admin action (`Form.clone()`) re-uses a form for a recurring event (yearly school parties): it deep-copies parts, slots, asset images, and texts but drops all signups, `FORM_ACCESS`, and the share token, and reopens signups. Asset `[[asset:<id>]]` body references are rewritten to the cloned asset ids.

**Onboarding is much simpler than for lists**: one **broadcast link** (`/forms/s/<share_token>/`, the `shared` view) is mailed to many recipients. Viewing needs no login; signing up does. An anonymous visitor's signup button hands off via `shared_signin` (stashes `pending_form_token`, bounces to login/registration); after passkey login/enrolment `accounts.views._next_url_after_auth` returns them to the form. The token authorises signup even without a prior `FORM_ACCESS` (see `_can_user_signup`). There is **no per-recipient invite token** (unlike `LIST_INVITE_TOKEN`) — the share link is reusable. There is exactly **one token per form**, created lazily the first time a form admin opens the detail view (no explicit "generate link" action) and shown read-only at the top of that page for the admin to copy.
