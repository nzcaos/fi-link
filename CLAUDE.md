...
**Member-attribute grid (`LIST_ATTRIBUT.display_row`).** The child's `member` attributes are not stacked with inline labels — they render as a grid beside the (vertically-centred, far-left) child name, columns side by side with the label shown once above each column, like the associate columns. Because not all child attributes fit on one line, each attribute carries a `display_row` (1, 2, …, default 1) on the LISTTEMPLATE that places it on a given grid row; within a row, `position` orders the columns. The composer (`build_composed_rows`) groups the member attributes into `member_grid` (one list of `{attr, value}` cells per display-row, `value=None` where hidden → "—"); `lists/_child_cell.html` renders it. Associate attributes keep their single top-level header row (one column per attribute) unchanged.
